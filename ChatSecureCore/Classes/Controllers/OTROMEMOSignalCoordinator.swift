...
//                let messageBodyData = messageBody.data(using: String.Encoding.utf8) else {
//                return
//            }
            
            guard let msgText = message.messageText,
                let ivData = OTRSignalEncryptionHelper.generateIV(),
                let keyData = OTRSignalEncryptionHelper.generateSymmetricKey() else {
                return
            }
            
            var msgBody = msgText
            if let fireTimerStr = fireTimerString, fireTimerStr.count > 0 {
